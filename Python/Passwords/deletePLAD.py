import csv, createPLAD, decryptPLAD

def main(argv,n,e):
    key_name_to_delete = getRequirements(argv)
    p, q, private_key_d = decryptPLAD.recivesKeys(n)
    correct_key_d = decryptPLAD.analizePrivateMaterKey(n, e, p, q, private_key_d)

    if correct_key_d == True:
        try:
            deleteByKeyName(key_name_to_delete)
            createPLAD.printPLADlogo()
...
def deleteByKeyName(searching_key_name):
    ...
    with open("dbPLAD.csv", "r") as searchingFile:
    ...
    with open("dbPLAD.csv","w") as new_file:
