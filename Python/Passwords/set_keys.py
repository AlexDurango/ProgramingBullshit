import csv, random, encryptPLAD,createPLAD

def main():    
    e = 65537
    oldData = []

    with open("dbPLAD.csv", "r") as keys:
    ...
    with open("dbPLAD.csv", "w") as newKeys:
        writer = csv.writer(newKeys)
        writer.writerow(publicKeys)
        writer.writerows(oldData)


    landaN = encryptPLAD.lamdaN(p, q)
    private_key_d = encryptPLAD.generatePrivateKey(landaN, e)
